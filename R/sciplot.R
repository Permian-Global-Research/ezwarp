...
#' @param pal Default is 'acton'. Character describing colour palette to use.
#' choose from `sciplot_pals()`
#' @param n Numeric of length 1. The number of breaks/colours to include in the
#' sclae
#' @param direction Default is 1. Direction of colour palette to reverse use -1.
#' @param centre Logical. If TRUE, the palette is centred around zero. good for
#' bivariate/split palettes.
#' @param n_quantile Numeric. if not NULL sets the number of quantile breaks to
#' use for sciplot.SpatRaster. See details.
...
#' @rdname sciplot
#' @family plot_helpers
#' @return character vector of scico palette names.
...
include_cols <- function(x, mav, n) {
  ...
  min(which(n_in_class == TRUE)):max(which(n_in_class == TRUE))
}
