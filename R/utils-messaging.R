...
engine_error <- function(engine) {
  cli::cli_abort(
    c(
      "{engine} is not a supported {.var engine}",
      "i" = paste0(
        "choose from",
        cli::style_bold(" vapour "),
        "or",
        cli::style_bold(" sf")
      )
    )
...
band_mismatch_error <- function() {
  cli::cli_abort(c(
    "Input raster source band numbers differ for `x`.",
...
not_list_error <- function(x) {
  cli::cli_abort(
    c(
      paste0(
        "When > 1 source is provided for {.var x},
...
template_length_error <- function(y) {
  cli::cli_abort(
    c(
      "Only one spatial source can be provided for {.var y}.",
...
spat_source_error <- function(x) {
  cli::cli_abort(c(
    "Source can't be read by gdal or ogr",
